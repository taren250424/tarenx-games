...
- A new game joins the hub on its own: `<meta name="hub:tagline">` and `<meta name="hub:category">` in its index.html put its card on the list, and `markPlayed()` from `packages/shared/progress/recent.ts` on load puts it in "Recently played". When a round ends it calls `recordResult()` from `packages/shared/progress/stats.ts` and drops `statGrid()` into its `.result` sheet, which is what the hub reads for the card's played count. Nothing errors when they are missing — the card just goes quiet.
