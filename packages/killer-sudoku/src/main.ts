...
const STORAGE_KEY = "tarenx.killer-sudoku.progress";
...
const cageInfoEl = document.getElementById("cage-info") as HTMLElement;
...
// --- cages ---
// The layout names each cell's cage by a character; cages are numbered in
// order of first appearance, so a cage's label sits on its first cell.
function parseCages(layout: string): void {
	...
	for (let i = 0; i < 81; i++) {
		...
		cages[id].cells.push(i);
		cages[id].sum += solution[i];
		cageOf[i] = id;
	}
}

// Every set of distinct digits that could still fill the cage's empty
// cells: none the cage already holds, adding up to what is left.
function combos(cage: Cage): number[][] {
	let need = cage.sum;
	let held = 0;
	let open = 0;
	...
	const out: number[][] = [];
	const walk = (from: number, left: number, sum: number, picks: number[]) => {
		if (left === 0) {
			if (sum === need) out.push([...picks]);
			return;
		}
		for (let d = from; d <= 9; d++) {
			if (held & (1 << (d - 1)) || sum + d > need) continue;
			picks.push(d);
			walk(d + 1, left - 1, sum + d, picks);
			picks.pop();
		}
	};
	if (open > 0) walk(1, open, 0, []);
	return out;
}
...
function loadPuzzle(diff: Difficulty, index: number, session?: Session) {
	difficulty = diff;
	puzzleIndex = index;
	const puzzle = PUZZLES[diff][index];
	solution = [...puzzle.solution].map(Number);
	parseCages(puzzle.cages);
	values = session ? [...session.values].map(Number) : new Array(81).fill(0);
...
// A digit twice in a row, column, box or cage.
function conflictSet(): Set<number> {
	const conflicts = new Set<number>();
	for (let i = 0; i < 81; i++) {
		if (!values[i]) continue;
		for (const p of [...PEERS[i], ...cages[cageOf[i]].cells]) {
			if (p !== i && values[p] === values[i]) {
				conflicts.add(i);
				conflicts.add(p);
			}
		}
...
// A full cage whose digits miss its sum.
function cageOff(cage: Cage): boolean {
	let sum = 0;
	for (const c of cage.cells) {
...
function render() {
	...
	for (let i = 0; i < 81; i++) {
		...
		if (r % 3 === 0 && r !== 0) cls += " bt";
		if (c % 3 === 0 && c !== 0) cls += " bl";
		if (r === 0 || cageOf[i - 9] !== cageOf[i]) cls += " ct";
		if (r === 8 || cageOf[i + 9] !== cageOf[i]) cls += " cb";
		if (c === 0 || cageOf[i - 1] !== cageOf[i]) cls += " cl";
		if (c === 8 || cageOf[i + 1] !== cageOf[i]) cls += " cr";
		if (i === selected) cls += " selected";
		else if (selected >= 0 && (PEERS[i].includes(selected) || cageOf[i] === cageOf[selected])) cls += " peer";
		if (selectedValue && values[i] === selectedValue && i !== selected) cls += " same";
		if (conflicts.has(i)) cls += " conflict";
		if (progress.settings.mistakes && values[i] && values[i] !== solution[i]) cls += " wrong";

		let content = "";
		if (cage.cells[0] === i) {
			content += `<span class="sum${cageOff(cage) ? " bad" : ""}">${cage.sum}</span>`;
		}
		if (values[i]) {
...
function renderCageInfo() {
	if (selected < 0) {
		cageInfoEl.textContent = "Tap a cell to see what its cage can hold";
		return;
	}
	const cage = cages[cageOf[selected]];
	const open = cage.cells.filter((c) => !values[c]).length;
	const head = `<strong>${cage.sum}</strong> in ${cage.cells.length} · `;
	if (open === 0) {
		cageInfoEl.innerHTML = head + (cageOff(cage) ? "does not add up" : "complete");
		return;
	}
	const list = combos(cage);
	if (list.length === 0) {
		cageInfoEl.innerHTML = head + "no way to finish";
		return;
	}
	const left = open === cage.cells.length ? "" : `${open} left: `;
	cageInfoEl.innerHTML =
		head + left + list.map((combo) => `<span class="combo">${combo.join("+")}</span>`).join("");
}
...
function input(d: number) {
	...
	if (notesMode) {
		...
		values[selected] = d;
		notes[selected] = 0;
		// entering a digit clears it from the notes of every peer and cage mate
		for (const p of [...PEERS[selected], ...cages[cageOf[selected]].cells]) notes[p] &= ~(1 << (d - 1));
...
function hint() {
	...
	const target =
		selected >= 0 && values[selected] !== solution[selected]
			? selected
	...
	notes[target] = 0;
	for (const p of [...PEERS[target], ...cages[cageOf[target]].cells]) notes[p] &= ~(1 << (solution[target] - 1));
	selected = target;
	hintsLeft--;
	play("hint");
	saveSession();
...
async function nextPuzzle() {
	const [nd, ni] = nextPosition();
	await ads.interstitial("next", "killer-sudoku-next");
	loadPuzzle(nd, ni);
}
